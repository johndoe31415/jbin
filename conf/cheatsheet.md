...
## crypttab
Standard configuration for crypt-root:
```
crypt-root UUID=aaa none luks,tries=99,discard
```

Multiple partitions using decrypt-keyctl:
```
crypt-root UUID=aaa crypt_disks luks,tries=99,discard,keyscript=decrypt_keyctl,initramfs
crypt-ext UUID=bbb crypt_disks luks,tries=99,discard,keyscript=decrypt_keyctl,initramfs
```
